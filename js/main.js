...
const avgFPS = { "Warzone": document.getElementById('avgWarzone') };
let gameData = {};
let fpsChart;

async function loadGameData(game){
  const response = await fetch(`data/${game.toLowerCase()}.json`);
...
function updateFPS(){
  ...
  avgFPS["Warzone"].textContent = count?Math.floor(sum/count):0;
}
